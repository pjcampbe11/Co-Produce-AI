...
through `vst_chain.py` (pedalboard hosts them headlessly - no DAW).
...
| `destroy_extreme.json` | destroy_heal.py input | Dirt -> TR5 Tape 80 -> Thermal -> Pro-L 2 |
...
python scripts\vst_chain.py --list-params "C:\Program Files\Common Files\VST3\FabFilter\FabFilter Saturn 2.vst3"

:: open the GUI of chain item 0, dial it, close window -> applied to the batch
python scripts\vst_chain.py --input "F:\samples_in" --output "F:\samples_out" --chain configs\vst_chains\dusty_boombap.json --edit 0
```

## Batch process (drives can differ from where the repo lives)

```bat
python scripts\vst_chain.py --input "F:\RAP_ARCHIVES\raw_beats" --output "F:\RAP_ARCHIVES\beats_dusty" --chain configs\vst_chains\dusty_boombap.json
```
...
- Vocal removal: prefer `remove_vocals.py` (BS-RoFormer). Use
