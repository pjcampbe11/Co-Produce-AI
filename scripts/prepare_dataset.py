...
def main():
    ...
    ap.add_argument("--name-contains", default="", help="Only include files whose name contains this (e.g. _instrumental)")
    ap.add_argument("--name-excludes", default="", help="Skip files whose name contains this (e.g. _vocals)")
    args = ap.parse_args()
    ...
    exts = {".wav", ".flac", ".aif", ".aiff", ".mp3", ".ogg", ".m4a"}
    wavs = sorted(p for p in input_root.rglob("*") if p.suffix.lower() in exts)
    if args.name_contains:
        wavs = [w for w in wavs if args.name_contains.lower() in w.name.lower()]
    if args.name_excludes:
        wavs = [w for w in wavs if args.name_excludes.lower() not in w.name.lower()]
    if not wavs:
        sys.exit("No audio files found (after name filters).")
